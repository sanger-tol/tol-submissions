...
from main.model import SubmissionsState, SubmissionsUser, db
...
def apikey_auth(token, required_scopes):
    ...
    user = db.session.query(SubmissionsUser) \
        .filter(SubmissionsUser.api_key == token) \
        .one_or_none()

    if user is None:
        user = db.session.query(SubmissionsUser) \
            .filter(SubmissionsUser.token == token) \
            .one_or_none()
        if user is None:
...
def login():
    ...
    db.session.query(SubmissionsState) \
        .filter(SubmissionsState.created_at < since) \
        .delete()
    db.session.commit()
...
def get_token_from_callback(body=None):
    ...
    state_from_db = db.session.query(SubmissionsState) \
        .filter(SubmissionsState.state == body['state']) \
        .one_or_none()
...
def create_user_profile(body=None):
    ...
    if user_info_from_elixir.get('error') is None:
        user = db.session.query(SubmissionsUser) \
            .filter(SubmissionsUser.email == user_info_from_elixir['email']) \
            .one_or_none()
        if not user:
